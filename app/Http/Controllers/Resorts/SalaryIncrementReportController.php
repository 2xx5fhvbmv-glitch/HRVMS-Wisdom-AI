...
class SalaryIncrementReportController extends Controller
{
    ...
    public function salaryIncrementRegister(array $f): array
    {
        ...
        $rows = $this->baseQuery($rid, $scoped)
            ...
            ->map(fn($r) => [
                ...
                'Current Salary'        => $r->previous_salary !== null ? Common::formatCurrency($r->previous_salary) : 'N/A',
                'New Salary'            => $r->new_salary !== null ? Common::formatCurrency($r->new_salary) : 'N/A',
                'Increment Amount'      => $r->increment_amount !== null ? Common::formatCurrency($r->increment_amount) : 'N/A',
    ...
    public function employeeSalaryIncrementHistory(array $f): array
    {
        ...
        $rows = $this->baseQuery($rid, $scoped)
            ...
            ->map(fn($r) => [
                ...
                'Department'            => $r->dept_name ?? 'N/A',
                'Position'              => $r->position_title ?? 'N/A',
                'Previous Salary'       => $r->previous_salary !== null ? Common::formatCurrency($r->previous_salary) : 'N/A',
                'Revised Salary'        => $r->new_salary !== null ? Common::formatCurrency($r->new_salary) : 'N/A',
                'Increment Amount'      => $r->increment_amount !== null ? Common::formatCurrency($r->increment_amount) : 'N/A',
                'Increment Percentage'  => ($pct = $this->pctFor($r->pay_increase_type, $r->value, $r->increment_amount, $r->previous_salary)) !== null ? $pct . '%' : 'N/A',
                'Increment Type'        => $r->increment_type ?: 'N/A',
                'Effective Date'        => $r->effective_date ? Carbon::parse($r->effective_date)->format('d M Y') : 'N/A',
    ...
    public function pendingSalaryIncrementApproval(array $f): array
    {
        ...
        $rows = $this->baseQuery($rid, $scoped)
            ...
            ->map(fn($r) => [
                ...
                'Position'              => $r->position_title ?? 'N/A',
                'Current Salary'        => $r->previous_salary !== null ? Common::formatCurrency($r->previous_salary) : 'N/A',
                'Proposed Salary'       => $r->new_salary !== null ? Common::formatCurrency($r->new_salary) : 'N/A',
                'Increment Amount'      => $r->increment_amount !== null ? Common::formatCurrency($r->increment_amount) : 'N/A',
                'Increment Percentage'  => ($pct = $this->pctFor($r->pay_increase_type, $r->value, $r->increment_amount, $r->previous_salary)) !== null ? $pct . '%' : 'N/A',
                'Effective Date'        => $r->effective_date ? Carbon::parse($r->effective_date)->format('d M Y') : 'N/A',
    ...
    public function salaryIncrementFinancialImpact(array $f): array
    {
        ...
        $rowsOut = [[
            ...
            'Total Current Basic Salary'        => Common::formatCurrency($totalCurrent),
            'Total Proposed Basic Salary'       => Common::formatCurrency($totalProposed),
            'Total Monthly Payroll Increase'    => Common::formatCurrency($totalMonthlyIncrease),
            'Total Annual Payroll Increase'     => Common::formatCurrency($totalAnnualIncrease),
            'Average Increment Amount'          => $avgIncrementAmount !== null ? Common::formatCurrency($avgIncrementAmount) : 'N/A',
