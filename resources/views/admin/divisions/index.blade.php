...
@section('page_tab_title' ,"Divisions")
            ...
            <div class="card-header">
              ...
              <div>
                @if(Common::hasPermission(config('settings.admin_modules.divisions'),config('settings.permissions.create')))
                  <a href="{{route('admin.divisions.create')}}" class="btn btn-primary float-right"><i class="fas fa-plus"></i> Add</a>
                @endif
...
<script>
  $(document).ready(function() {
    ...
    $("#all_chk").click(function() {
      if ($(this).is(':checked')) {
        var $view = $('.division_checkbox').prop("checked", true);
      } else {
        var $view = $('.division_checkbox').prop("checked", false);
      }
    });

    $(document).on('click', '.division_checkbox', function() {
      var numberOfChecked = $('.division_checkbox').filter(':checked').length;
      var totalCheckboxes = $('.division_checkbox').length;
    ...
    $(document).on('click', '#bulk_delete', function() {
      ...
      if (id.length > 0) {
        swal({
          ...
          if (success) {
            $.ajax({
              url: "{{route('admin.divisions.massremove')}}",
              data: {
                id: id
              },
              dataType: "json",
