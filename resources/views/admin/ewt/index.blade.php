        ...
        <div class="col-12">
          <div class="card">
            <div class="card-header">
              <h1>EWT Brackets</h1>
              <div>
                @if(Common::hasPermission(config('settings.admin_modules.ewt_brackets'),config('settings.permissions.create')))
                  <a href="{{route('admin.ewt_brackets.create')}}" class="btn btn-primary float-right"><i class="fas fa-plus"></i> Add</a>
                @endif
...
<script>
  $(document).ready(function() {
    var url = "{{route('admin.ewt_brackets.list')}}";
    ...
    $("#all_chk").click(function() {
      if ($(this).is(':checked')) {
        var $view = $('.ewt_checkbox').prop("checked", true);
      } else {
        var $view = $('.ewt_checkbox').prop("checked", false);
      }
    });

    $(document).on('click', '.ewt_checkbox', function() {
      var numberOfChecked = $('.ewt_checkbox').filter(':checked').length;
      var totalCheckboxes = $('.ewt_checkbox').length;
    ...
    $(document).on('click', '#bulk_delete', function() {
      ...
      $('.ewt_checkbox:checked').each(function() {
        id.push($(this).val());
      });
      ...
      if (id.length > 0) {
        swal({
          ...
          if (success) {
            $.ajax({
              url: "{{route('admin.ewt_brackets.massremove')}}",
              data: {
                id: id
              },
              dataType: "json",
