        ...
        <div class="col-12">
          <div class="card">
            <div class="card-header">
              <h1>Resort Modules </h1>
              <div>
                  <a href="{{route('admin.Modules.create')}}" class="btn btn-primary float-right"><i class="fas fa-plus"></i> Add</a>
...
<script>
  $(document).ready(function() {
    var url = "{{route('admin.Modules.index')}}";
    ...
    $("#all_chk").click(function() {
      if ($(this).is(':checked')) {
        var $view = $('.Module_checkbox').prop("checked", true);
      } else {
        var $view = $('.Module_checkbox').prop("checked", false);
      }
    });

    $(document).on('click', '.Module_checkbox', function() {
      var numberOfChecked = $('.Module_checkbox').filter(':checked').length;
      var totalCheckboxes = $('.Module_checkbox').length;
    ...
    $(document).on('click', '#bulk_delete', function() {
      ...
      $('.Module_checkbox:checked').each(function() {
        id.push($(this).val());
      });
      ...
      if (id.length > 0) {
        swal({
          ...
          if (success) {
            $.ajax({
              url: "{{route('admin.Modules.massremove')}}",
              data: {
                id: id
              },
              dataType: "json",
