...
@section('page_tab_title' ,"Departments")
        ...
        <div class="col-12">
          <div class="card">
            <div class="card-header">
              <h1>Departments</h1>
              <div>
                @if(Common::hasPermission(config('settings.admin_modules.departments'),config('settings.permissions.create')))
                  <a href="{{route('admin.department.create')}}" class="btn btn-primary float-right"><i class="fas fa-plus"></i> Add</a>
                @endif
                ...
                @endif
                @if(Common::hasPermission(config('settings.admin_modules.departments'),config('settings.permissions.create')))
                  @include('admin._partials.bulk-upload', ['module' => 'department', 'sampleRoute' => 'admin.department.sample', 'importRoute' => 'admin.department.import', 'entityLabel' => 'Department'])
                @endif
              </div>
            </div>
            <div class="card-body">
...
<script>
  $(document).ready(function() {
    var url = "{{route('admin.department.list')}}";
    ...
    $("#all_chk").click(function() {
      if ($(this).is(':checked')) {
        var $view = $('.department_checkbox').prop("checked", true);
      } else {
        var $view = $('.department_checkbox').prop("checked", false);
      }
    });

    $(document).on('click', '.department_checkbox', function() {
      var numberOfChecked = $('.department_checkbox').filter(':checked').length;
      var totalCheckboxes = $('.department_checkbox').length;
    ...
    $(document).on('click', '#bulk_delete', function() {
      ...
      if (id.length > 0) {
        swal({
          ...
          if (success) {
            $.ajax({
              url: "{{route('admin.department.massremove')}}",
              data: {
                id: id
              },
              dataType: "json",
