...
@section('page_tab_title' ,"Sections")
        ...
        <div class="col-12">
          <div class="card">
            <div class="card-header">
              <h1>Sections</h1>
              <div>
                @if(Common::hasPermission(config('settings.admin_modules.sections'),config('settings.permissions.create')))
                  <a href="{{route('admin.sections.create')}}" class="btn btn-primary float-right"><i class="fas fa-plus"></i> Add</a>
                @endif
                ...
                @endif
                @if(Common::hasPermission(config('settings.admin_modules.sections'),config('settings.permissions.create')))
                  @include('admin._partials.bulk-upload', ['module' => 'sections', 'sampleRoute' => 'admin.sections.sample', 'importRoute' => 'admin.sections.import', 'entityLabel' => 'Section'])
                @endif
              </div>
            </div>
            <div class="card-body">
...
<script>
  $(document).ready(function() {
    ...
    $("#all_chk").click(function() {
      if ($(this).is(':checked')) {
        var $view = $('.section_checkbox').prop("checked", true);
      } else {
        var $view = $('.section_checkbox').prop("checked", false);
      }
    });

    $(document).on('click', '.section_checkbox', function() {
      var numberOfChecked = $('.section_checkbox').filter(':checked').length;
      var totalCheckboxes = $('.section_checkbox').length;
    ...
    $(document).on('click', '#bulk_delete', function() {
      ...
      if (id.length > 0) {
        swal({
          ...
          if (success) {
            $.ajax({
              url: "{{route('admin.sections.massremove')}}",
              data: {
                id: id
              },
              dataType: "json",
