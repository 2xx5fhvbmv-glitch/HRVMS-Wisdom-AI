...
@section('page_tab_title' ,"Notifications")
        ...
        <div class="col-12">
          <div class="card">
            <div class="card-header">
              <h1>Notifications</h1>
              <div>
                @if(Common::hasPermission(config('settings.admin_modules.notifications'),config('settings.permissions.create')))
                  <a href="{{route('admin.notifications.create')}}" class="btn btn-primary float-right"><i class="fas fa-plus"></i> Add</a>
                @endif
...
<script>
  $(document).ready(function() {
    var url = "{{route('admin.notifications.list')}}";
    ...
    $("#all_chk").click(function() {
      if ($(this).is(':checked')) {
        var $view = $('.notice_checkbox').prop("checked", true);
      } else {
        var $view = $('.notice_checkbox').prop("checked", false);
      }
    });

    $(document).on('click', '.notice_checkbox', function() {
      var numberOfChecked = $('.notice_checkbox').filter(':checked').length;
      var totalCheckboxes = $('.notice_checkbox').length;
    ...
    $(document).on('click', '#bulk_delete', function() {
      ...
      $('.notice_checkbox:checked').each(function() {
        id.push($(this).val());
      });
      ...
      if (id.length > 0) {
        swal({
          ...
          if (success) {
            $.ajax({
              url: "{{route('admin.notifications.massremove')}}",
              data: {
                id: id
              },
              dataType: "json",
