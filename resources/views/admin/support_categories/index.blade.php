...
@section('page_tab_title' ,"Support Categories")
        ...
        <div class="col-12">
          <div class="card">
            <div class="card-header">
              ...
              <div>
                  <a href="{{route('admin.support_categories.create')}}" class="btn btn-primary float-right"><i class="fas fa-plus"></i> Add</a>
...
<script>
  $(document).ready(function() {
    var url = "{{ route('admin.support_categories.list') }}";

    var table = $('#datatable').DataTable({
        processing: true, // Keep it true for a better user experience
        serverSide: true,
        ajax: url,
        order: [[1, "asc"]], // Sort by name column
        columns: [
            { data: 'checkbox', name: 'checkbox', orderable: false, searchable: false },
            { data: 'name', name: 'name' }, // Ensure 'name' column exists in the backend
    ...
    $("#all_chk").click(function() {
      if ($(this).is(':checked')) {
        var $view = $('.support_category_checkbox').prop("checked", true);
      } else {
        var $view = $('.support_category_checkbox').prop("checked", false);
      }
    });

    $(document).on('click', '.support_category_checkbox', function() {
      var numberOfChecked = $('.support_category_checkbox').filter(':checked').length;
      var totalCheckboxes = $('.support_category_checkbox').length;
    ...
    $(document).on('click', '#bulk_delete', function() {
      ...
      $('.support_category_checkbox:checked').each(function() {
        id.push($(this).val());
      });
      ...
      if (id.length > 0) {
        swal({
          ...
          if (success) {
            $.ajax({
              url: "{{route('admin.support_categories.massremove')}}",
              data: {
                id: id
              },
              dataType: "json",
