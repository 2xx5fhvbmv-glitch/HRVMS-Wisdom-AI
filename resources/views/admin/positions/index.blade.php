        ...
        <div class="col-12">
          <div class="card">
            <div class="card-header">
              <h1>Positions</h1>
              <div>
                @if(Common::hasPermission(config('settings.admin_modules.positions'),config('settings.permissions.create')))
                  <a href="{{route('admin.positions.create')}}" class="btn btn-primary float-right"><i class="fas fa-plus"></i> Add</a>
                @endif
                ...
                @endif
                @if(Common::hasPermission(config('settings.admin_modules.positions'),config('settings.permissions.create')))
                  @include('admin._partials.bulk-upload', ['module' => 'positions', 'sampleRoute' => 'admin.positions.sample', 'importRoute' => 'admin.positions.import', 'entityLabel' => 'Position'])
                @endif
              </div>
            </div>
            <div class="card-body">
...
<script>
  $(document).ready(function() {
    var url = "{{route('admin.positions.list')}}";
    ...
    $("#all_chk").click(function() {
      if ($(this).is(':checked')) {
        var $view = $('.position_checkbox').prop("checked", true);
      } else {
        var $view = $('.position_checkbox').prop("checked", false);
      }
    });

    $(document).on('click', '.position_checkbox', function() {
      var numberOfChecked = $('.position_checkbox').filter(':checked').length;
      var totalCheckboxes = $('.position_checkbox').length;
    ...
    $(document).on('click', '#bulk_delete', function() {
      ...
      $('.position_checkbox:checked').each(function() {
        id.push($(this).val());
      });
      ...
      if (id.length > 0) {
        swal({
          ...
          if (success) {
            $.ajax({
              url: "{{route('admin.positions.massremove')}}",
              data: {
                id: id
              },
              dataType: "json",
