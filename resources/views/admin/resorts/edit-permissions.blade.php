            ...
            <div class="card-body">
              <form method="POST" id="frmEditPermissions" class="form-horizontal" action="{{ route('admin.resorts.update_permissions', $resort_id) }}">
                  ...
                  <div class="row">
                    <div class="col-md-12">
                        <div id="accordion">
                            @if($Resortmodules->isNotEmpty())
                                @foreach ($Resortmodules as $modules)
                                    <div class="card">
                                        <div class="card-header" id="heading_{{$modules->id}}">
                                            <h5 class="mb-0">
                                                <button class="btn btn-link collapsed" type="button" data-toggle="collapse" data-target="#collapse_{{$modules->id}}" aria-expanded="false" aria-controls="collapse_{{$modules->id}}">
                                                    {{ $modules->module_name }}  
                                                </button>
                                            </h5>
                                        </div>
                                        ...
                                        <div id="collapse_{{$modules->id}}" class="collapse" aria-labelledby="heading_{{$modules->id}}" data-parent="#accordion">
                                            <div class="card-body">
                                                <table class='table table-responsive border'>
                                                    <tbody>
                                                        @if($pages->isNotEmpty())
                                                            <tr>
                                                                <td>
                                                                    <input class="form-check-input parent-checkbox" data-id="{{$modules->id}}" id="parent_{{ $modules->id }}" name="module_permissions_parent" type="checkbox" value="">
                                                                    <label for="parent_{{ $modules->id }}">Select All</label>
                                                                </td>
                                                            </tr>
                                                        @else
                                                            <tr>
                                                                ...
                                                                @foreach ($pages as $key=>$p)
                                                                    <td>
                                                                        <div class="custom-checkbox-container">
                                                                            <input class="form-check-input child-checkbox child_parent_{{$modules->id}}"
                                                                            @if(array_key_exists($modules->id,$ModuleWisePermission) && in_array($p->id ,$ModuleWisePermission[$modules->id]))
                                                                            checked
                                                                            @endif  id="child_{{ $p->id }}" name="module_permissions[{{ $modules->id }}][]" type="checkbox" value="{{ $p->id }}">
                                                                            <label for="child_{{ $p->id }}" class="custom-checkbox-label">
                                                                                {{ $p->page_name }}
                                                                            </label>
                                                                        </div>
                                                                    </td>
                      ...
                      <div class="col-12">
                          <div class="row g-lg-4 g-3">
                              <div class="col-auto">
                                  ...
                                  <a href="{{ route('admin.resorts.index') }}" class="btn btn-outline-info">Cancel</a>
                              </div>
                          </div>
                      </div>
                  </div>
