					...
					<div class="card">
						<div class="card-header">
							@if(Common::hasPermission(config('settings.admin_modules.roles_permissions'),config('settings.permissions.create')))
								<a href="{{route('admin.role.create')}}" class="btn btn-primary float-right"><i class="fas fa-plus"></i> Add</a>
							@endif
	  ...
	  $(document).on('click', '#bulk_delete', function(){
    	...
    	if( id.length > 0 ) {
    		swal({
    			...
    			if (success) {
    				$.ajax({
    					url: "{{route('admin.role.massremove')}}",
    					data: {id:id},
    					dataType: "json",
    					beforeSend: function(msg) {
    						HoldOn.open({
