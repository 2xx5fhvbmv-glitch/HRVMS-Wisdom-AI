            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="crop-container ">
                        ...
                        <button id="close-crop-btn">Close</button>
                    </div>
                </div>
                <div class="modal-footer">
    ...
    for (let i = 0; i < fileList.length; i++) {
        ...
        if (file.type.startsWith('image/')) {
            ...
            fileReader.onload = function(e) {
                ...
                row.innerHTML = `
                    <td>${file.name}</td>
                    <td><a type="button"  href="#CropImage-modal"  data-bs-toggle="modal"
                        class="btn btn-sm btn-themeSkyblue crop-btn" data-index="${i}">Crop</a></td>
                `;
                fileListContainer.appendChild(row);

                // Attach crop button event
                row.querySelector('.crop-btn').addEventListener('click', function () {
                    openCropModal(file, i);
                });
            }
...
function openCropModal(file, index) {
    ...
    cropImage.onload = function() {
        // First, set some inline styles to ensure image fits container
        cropImage.style.maxHeight = '100%';
        cropImage.style.maxWidth = '100%';
        
        cropper = new Cropper(cropImage, {
            aspectRatio: NaN,      // Free aspect ratio
            viewMode: 0,           // No restrictions on the crop box size
            dragMode: 'crop',      // Set initial drag mode to crop
            autoCropArea: 1,       // Use 100% of the image
            restore: false,
            guides: true,
            center: true,
            highlight: true,
            ...
            // Apply this class to change how the cropper appears
            modalClass: 'cropper-modal-black',
            // Force crop box to cover whole image on initialization
            ready: function() {
                // Get image data dimensions
                const imageData = cropper.getImageData();
                // Set crop box to match image dimensions exactly
                ...
                // Force the cropper to recalculate and fit the container
                cropper.resize();
            }
        });
