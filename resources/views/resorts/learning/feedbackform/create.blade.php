                                        ...
                                        <select name="training" id="training" class="select2t-none">
                                            ...
                                            @if($trainings)
                                                @foreach($trainings as $training)
                                                    <option value="{{$training->id}}">{{$training->learningProgram->name}}</option>
                                                @endforeach
                                            @endif
                                        </select>
                                    </div> -->
...
@section('import-scripts')
    ...
    <script>
        $(document).ready(function () {
            ...
            // Custom field templates — same ones used by /resort/performance/configuration so
            // feedback forms can also include star ratings and rating tables.
            var templates = {
            ...
            $('#save-form').click(function () {
                ...
                $.ajax({
                    url: "{{ route('feedback-form.store') }}",
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                        // position : $('#training').val(),
