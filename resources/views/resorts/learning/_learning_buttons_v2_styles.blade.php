...
<style>
    ...
    /* Main forward action — Submit/Save/Update/Schedule. */
    .lnd-btn-primary { background: #014653; color: #fff; }
    .lnd-btn-primary:hover { background: #014653; color: #fff; transform: translateY(-2px); box-shadow: 0 8px 18px -8px rgba(20,35,42,.35); }

    /* Low-stakes, recedes on purpose — View/Back/Cancel-nav. */
    .lnd-btn-secondary { background: transparent; color: #014653; border: 1.5px solid #C9D6D7; }
    ...
    .lnd-btn-accent { background: #E6F0F1; color: #035b6c; }
    .lnd-btn-accent:hover { background: #035b6c; color: #fff; transform: translateY(-2px); box-shadow: 0 8px 18px -8px rgba(20,35,42,.25); }

    /* The brand lime — reserved for a single standout hero CTA per page,
       not blanketed across every additive button. */
    .lnd-btn-hero { background: #E0FF02; color: #17260a; }
    .lnd-btn-hero:hover { background: #E0FF02; color: #17260a; transform: translateY(-2px); box-shadow: 0 8px 18px -8px rgba(20,35,42,.35); }

    /* Backing out / low-stakes state change, not a decision — Cancel,
       On Hold, Remove an unsaved row, Back to Dashboard. */
    .lnd-btn-neutral { background: #DEDEDE; color: #222; }
    .lnd-btn-neutral:hover { background: #F5F8F8; color: #222; transform: translateY(-2px); box-shadow: 0 8px 18px -8px rgba(20,35,42,.18); }

    /* The hard-stop decision — Deny, Remove, Delete. */
    .lnd-btn-critical { background: #FFDED9; color: #FF2400; }
    .lnd-btn-critical:hover { background: #FF2400; color: #fff; transform: translateY(-2px); box-shadow: 0 8px 18px -8px rgba(20,35,42,.25); }
    ...
    .lnd-btn-critical[disabled] { background: #FFDED9; color: #FF2400; }
    ...
    /* Row-action icon for the hard-stop decision (Delete/Remove) — the
       ...
       global class other modules still rely on. */
    .lnd-icon-critical { color: #FF2400; background: rgba(255,36,0,.09); }
    .lnd-icon-critical:hover { color: #fff; background: #FF2400; }
</style>
