        ...
        <div class="container-fluid">
            ...
            <div class="row g-3 g-xxl-4 card-heigth card-incidentHr">
                ...
                <div class="col-xl-3 col-sm-6">
                    <div class="card card-wiINsightVisa wai-narrative h-100" id="card-wiINsightVisa">
                        @php $vMeta = $visaInsights['_meta'] ?? null; @endphp
                        <div class="wai-head">
                            <h2>WAI Insights</h2>
                            @if($vMeta)
                                <div class="wai-head-meta">
                                    <span>Updated {{ $vMeta['generated_at']->diffForHumans() }}</span>
                                    @if($vMeta['can_regenerate'])
                                        <a href="?regenerate_insights=1">Regenerate</a>
                                    @else
                                        <span title="{{ $vMeta['next_available']->format('d M Y, H:i') }}">&middot; Regenerate {{ $vMeta['next_available']->diffForHumans() }}</span>
                                    @endif
                                </div>
                            @endif
                        </div>
                        <div class="leaveUser-main wai-narrative-body">
                            @foreach([['key'=>'payments','modal'=>'visaInsightPaymentsModal'],['key'=>'liability','modal'=>'visaInsightLiabilityModal'],['key'=>'renewal','modal'=>'visaInsightRenewalModal'],['key'=>'expiry','modal'=>'visaInsightExpiryModal']] as $vc)
                                @php $hasRecommendation = !empty($visaInsights[$vc['key']]['recommendation']); @endphp
                                <div class="wai-row">
