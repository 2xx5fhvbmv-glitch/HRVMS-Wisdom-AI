...
<script type="text/javascript">
        ...
        $(".next").click(async function (e) {
            ...
            if (currentStep === 1) {
                ...
                if (!dateRange || dates.length !== 2) {
                    toastr.error("Please select a valid payroll period before proceeding.", {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.success) {
                            toastr.success("Payroll draft saved successfully!", {
                                positionClass: 'toast-bottom-right'
                            });
                    ...
                    error: function () {
                        toastr.error("Error saving payroll draft.", {
                            positionClass: 'toast-bottom-right'
                        });
                    }
                });
            ...
            if (currentStep === 2) {
                ...
                if (!payrollId) {
                    toastr.error("Payroll draft not found. Please start again.", {
                        positionClass: 'toast-bottom-right'
                    });
                    return;
                }
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.success) {
                            toastr.success("Employees added to payroll successfully!", {
                                positionClass: 'toast-bottom-right'
                            });
                            getstep3data(searchTerm="");
                            localStorage.setItem("selectedEmployees", selectedEmployees);
                            localStorage.setItem("currentStep", currentStep);
                    ...
                    error: function () {
                        toastr.error("Error saving employees.", {
                            positionClass: 'toast-bottom-right'
                        });
                    }
                });
            ...
            if (currentStep === 3) {
                ...
                if (AttendaceData.length === 0) {
                    toastr.error("Something is wrong to fetch attendance data.", {
                        positionClass: 'toast-bottom-right'
                    });
                    return;
                }
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.success) {
                            toastr.success("Attendance added to payroll successfully!", {
                                positionClass: 'toast-bottom-right'
                            });
            ...
            // Step 3: Save Service charges for employees
            if (currentStep === 4) {
                ...
                ServiceChargesDatae.totalAmount = parseFloat($("#total-ser").val()) || 0;
                ...
                $("#table-serviceCharge tbody tr").each(function () {
                    ...
                    ServiceChargesData.push({
                        ...
                        serviceCharge: $(this).find("td:eq(6)").text().trim() || 0 // Distributed Service Charge
                    });
                });

                if (ServiceChargesData.length === 0) {
                    toastr.error("Please distribute service charge", {
                        positionClass: 'toast-bottom-right'
                    });
                    return;
                }
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.success) {
                            toastr.success("Service Charges added to payroll successfully!", {
                                positionClass: 'toast-bottom-right'
                            });
                            
                           // Load currency rates before moving to the next step
                            loadCurrencyRates(ServiceChargesData ,resortid, currency, currentStep).then(() => {
                                moveToNextStep($currentFieldset);
                            }).catch(error => {
                                console.error("Error loading currency rates:", error);
                                toastr.error("Failed to load currency rates.", {
    ...
    $(document).on("click", ".edit-btn", function (e) {
        ...
        $(this).replaceWith('<a href="#" class="btn-lg-icon icon-bg-green save-btn">Save</a>');
    });
