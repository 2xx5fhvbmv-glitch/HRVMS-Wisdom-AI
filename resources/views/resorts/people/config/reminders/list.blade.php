                        ...
                        <table id="ReminderTable" class="table w-100">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-nowrap">Reminder (DAYS BEFORE)</th>
...
<script>
    ...
    function fecth(){
        ...
        var ReminderTable = $('#ReminderTable').DataTable({
            ...
            order:[[3, 'desc']],
            serverSide: true,
            ajax: '{{ route("people.reminders.list") }}',
            columns: [
                { data: 'task', name: 'task', className: 'text-nowrap' }, // Fixed column name
                { data: 'days', name: 'days'}, 
    ...
    $(document).on('click', '.delete-row-btn', function (e) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: "DELETE",
                    url: "{{ route('people.reminders.destroy', ':id') }}".replace(':id', main_id),
                    dataType: "json",
    ...
    $(document).on("click", "#ReminderTable .edit-row-btn", function (event) {
        ...
        var task = $row.find("td:nth-child(1)").text().trim();
        var days = $row.find("td:nth-child(2)").text().trim();

        var editRowHtml = `
            <td class="py-1">
                <div class="form-group">
                    <input type="text" class="form-control task" value="${task}" />
                </div>
            </td>
            <td class="py-1">
                <div class="form-group">
                    <input type="text" class="form-control days" value="${days}" />
                </div>
            </td>
            <td class="py-1">
    ...
    $(document).on("click", "#ReminderTable .update-row-btn", function (event) {
        ...
        var $row = $(this).closest("tr");
        var Main_id = $(this).attr('data-id');
        var task = $row.find("input.task").val();
