                ...
                <div class="card">
                    ...
                    <div class="AppendHerer">
                        <div class="talentAc-main talentAcQues-block">
                            <div class="talentAc-block mb-3">
                                <input type="hidden" name="editQuestionnaire_id" value="{{$Questionnaire->id}}" >
                                            ...
                                            <select class="form-select ResortDivision   ResortDivision" required data-id="1" name="ResortDivision" id="ResortDivision" aria-label="Default select example">
                                                ...
                                                @if($ResortDivision->isNotEmpty())
                                                    ...
                                                    @foreach ($ResortDivision as $d)
                                                        <option value="{{$d->id}}" {{ (isset($Questionnaire->Division_id) && $d->id == $Questionnaire->Division_id) ? 'selected' : '' }}>{{ $d->name}}</option>
                                                    @endforeach
                                                @endif
                                            ...
                                            <select class="form-select Department Department" required  data-id="1" name="Department" id="Department"  aria-label="Default select example">
                                                ...
                                                @if($ResortDpartments->isNotEmpty())
                                                    ...
                                                    @foreach ($ResortDpartments as $d)
                                                        <option value="{{$d->id}}" {{ (isset($Questionnaire->Department_id) && $d->id == $Questionnaire->Department_id) ? 'selected' : '' }}>{{ $d->name}}</option>
                                                    @endforeach
                                                @endif
                                            </select>
                                        </div>
                                            ...
                                            <select class="form-select Position" data-id="1" required name="Position"  id="Position" aria-label="Default select example">
                                                ...
                                                @if($ResortPositions->isNotEmpty())
                                                    @foreach ($ResortPositions as $d)
                                                        <option value="{{$d->id}}" {{ (isset($Questionnaire->Position_id) && $d->id == $Questionnaire->Position_id) ? 'selected' : '' }}>{{ $d->position_title}}</option>
                                                    @endforeach
                                                @endif
                                            </select>
                                        </div>
                                ...
                                @foreach ($Questionnaire->questionnaireChildren as $item)
                                    <div class="talentAc-block mb-3" id="remove_id_{{$iteration}}">
                                        <div class="title mb-2">
                                            ...
                                            <button type="button" class="btn ta-btn-critical btn-sm remove-btn"  data-id="{{$iteration}}">Remove</button>
                                        </div>
                                        @if($item->questionType == 'multiple')
                                            <div class="col-12 select_option select_multiple" style="">
                                                <div class="row gx-md-4 gx-3 g-2">
                                                    <div class="col-md-6">
                                                        <input type="text" class="form-control" value="{{$item->Question}}" placeholder="Question" name="AddQuestion[{{$iteration}}][]" fdprocessedid="bjkgk">
                                                    </div>
                                                    @php
                                                    ...
                                                    <div class="col-lg-5 col-md-4 col-sm-10">
                                                        <input type="number" class="form-control total-options" value="{{ $multipleCount }}"  data-id="{{$iteration}}"
                                                        placeholder="Total option number">

                                                            @if(!empty($options))
                                                                <ol class="listingNo-wrapper wrapper_{{$iteration}} mt-2 ">

                                                                    @foreach($options as $option)
                                                                            <li><input type="text" class="form-control" name="CheckBoxOption[{{$iteration}}][]" value="{{$option}}" placeholder="Check Box  Option {{$iteration}}" fdprocessedid="3bx02"></li>
                                                                    @endforeach
                                                                </ol>
                                                            @endif
                                                    </div>
                                                    <div class="col-lg-1 col-md-2 col-sm-2">
                                                        <input type="number" name="ans[]" value={{ $item->ans }} required class="form-control total-options_{{$iteration}}"
                                                        placeholder="Ans No">
                                                    </div>
                                                </div>
                                            </div>
                                        @elseif($item->questionType == 'Radio')
                                            <div class="col-12 select_option select_multiple" style="">
                                                <div class="row gx-md-4 gx-3 g-2">
                                                    <div class="col-md-6">
                                                        <input type="text" class="form-control" placeholder="Question"  value="{{$item->Question}}" name="AddQuestion[{{$iteration}}][]" fdprocessedid="bjkgk">
                                                    </div>
                                                    @php
                                                            $options = json_decode($item->options);
                                                    ...
                                                    @endphp
                                                    <div class="col-lg-5 col-md-4 col-sm-10">
                                                        <input type="number" class="form-control total-options" data-id="{{$iteration}}"
                                                        placeholder="Total Radio option number" required>
                                                        @if(!empty($options))
                                                        <ol class="listingNo-wrapper wrapper_{{$iteration}} mt-2 ">

                                                            @foreach($options as $option)
                                                                    <li><input type="text" class="form-control"  value="{{ $RadioCount }}" name="CheckBoxOption[{{$iteration}}][]" value="{{$option}}" placeholder="Check Box  Option {{$iteration}}" fdprocessedid="3bx02"></li>
                                                            @endforeach
                                                        </ol>
                                                    @endif
                                                    </div>
                                                    <div class="col-lg-1 col-md-2 col-sm-2">
                                                        <input type="number" name="ans[]" required class="form-control total-options_{{$iteration}}"
                                                        placeholder="Ans No">
                                                    </div>
                                                </div>
                                            </div>
                                        @else
                                            ...
                                            <div class="col-12 select_option select_text " data-id="{{$iteration}}">

                                                <input type="text" value="{{$item->Question}}" class="form-control" placeholder="Question" name="AddQuestion[{{$iteration}}][]" fdprocessedid="bjkgk">
                                            </div>
                                        @endif
                    ...
                    <div class="form-check form-switch form-switchTheme videoQuestions-switch mb-4">
                        <input class="form-check-input" type="checkbox" @if($Questionnaire->video == "Yes") checked  @endif name="video" role="switch" id="flexSwitchCheckDefault" @if($Questionnaire->video == 1) checked @endif>
                        <label class="form-check-label" for="flexSwitchCheckDefault">VIDEO QUESTIONS</label>
                    </div>


                    <div class="videoQuestions-main @if($Questionnaire->video != "Yes") d-none  @endif ">
                        @if($Questionnaire->questionnaireVideo->isNotEmpty())
                    ...
                    @if(isset($iteration))
                        @php $iteration = $iteration;  @endphp
                    @else
                    @php $iteration = 1;  @endphp

                    @endif
                    <input type="hidden" id="increment" value="{{$iteration}}">
        ...
        $(document).ready(function() {
            ...
            $('#StoreQuestionnaire').validate({
                ...
                submitHandler: function(form) {
                    ...
                    $.ajax({
                        url: "{{ route('resort.ta.update.Questionnaire') }}", // Ensure route is correct
                        type: "POST",
                        data: formData,
