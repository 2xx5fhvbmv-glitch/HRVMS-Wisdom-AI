...
<style>
    ...
    /* Main forward action — Submit/Save/Send, wizard Next/final Submit,
       Attach, Approve-request-style submits. Text stays literal #fff
       (contrast-on-solid-teal) — box-shadows stay literal
       rgba(20,35,42,…) throughout this file. */
    .perf-btn-primary { background: var(--teal); color: #fff; }
    .perf-btn-primary:hover { background: var(--teal); color: #fff; transform: translateY(-2px); box-shadow: 0 8px 18px -8px rgba(20,35,42,.35); }
    ...
    .perf-btn-secondary { background: transparent; color: var(--teal); border: 1.5px solid #C9D6D7; }
    .perf-btn-secondary:hover { background: var(--paper); border-color: var(--teal); color: var(--teal); transform: translateY(-2px); box-shadow: 0 8px 18px -8px rgba(20,35,42,.18); }
    ...
    .perf-btn-accent { background: var(--teal-3); color: var(--teal-2); }
    .perf-btn-accent:hover { background: var(--teal-2); color: #fff; transform: translateY(-2px); box-shadow: 0 8px 18px -8px rgba(20,35,42,.25); }
    ...
    .perf-btn-hero { background: var(--lime); color: #17260a; }
    .perf-btn-hero:hover { background: var(--lime); color: #17260a; transform: translateY(-2px); box-shadow: 0 8px 18px -8px rgba(20,35,42,.35); }

    /* Backing out / low-stakes state change, not a decision — Cancel,
       Close, Clear (filters), Archive, Remove an unsaved row. */
    .perf-btn-neutral { background: var(--neutral-bg); color: var(--darkblack); }
    .perf-btn-neutral:hover { background: var(--teal-soft); color: var(--darkblack); transform: translateY(-2px); box-shadow: 0 8px 18px -8px rgba(20,35,42,.18); }

    /* The hard-stop decision — Reject, Delete, Remove from PIP/PDP. */
    .perf-btn-critical { background: var(--critical-bg); color: var(--critical); }
    .perf-btn-critical:hover { background: var(--critical); color: #fff; transform: translateY(-2px); box-shadow: 0 8px 18px -8px rgba(20,35,42,.25); }
    ...
    .perf-btn-primary[disabled] { background: var(--teal); color: #fff; }
    .perf-btn-secondary[disabled] { background: transparent; color: var(--teal); }
    .perf-btn-positive[disabled] { background: var(--positive-bg); color: var(--positive); }
    .perf-btn-accent[disabled] { background: var(--teal-3); color: var(--teal-2); }
    .perf-btn-hero[disabled] { background: var(--lime); color: #17260a; }
    .perf-btn-neutral[disabled] { background: var(--neutral-bg); color: var(--darkblack); }
    .perf-btn-critical[disabled] { background: var(--critical-bg); color: var(--critical); }
    ...
    /* .page-hedding sits directly on the dark teal body::before band
       ...
       goes light-on-teal here instead. Left literal (not --card): this is
       contrast-on-the-teal-band white, not a surface color. */
