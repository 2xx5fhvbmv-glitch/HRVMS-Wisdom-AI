                    ...
                    <tbody>
                        @forelse($plans as $plan)
                            ...
                            <tr>
                                ...
                                <td>
                                    <a href="javascript:void(0)" class="btn-tableIcon btnIcon-danger pip-delete" data-id="{{ $plan->id }}"><i class="fa-regular fa-trash-can"></i></a>
                                </td>
                            </tr>
                        @empty
                            <tr><td colspan="8" class="text-center text-muted py-4">No employees in PIP</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
...
<script>
    $(document).ready(function() {
        ...
        // Delete
        $(document).on('click', '.pip-delete', function() {
            if (!confirm('Remove this employee from PIP?')) return;
            var id = $(this).data('id');
            var $row = $(this).closest('tr');
            $.ajax({
                url: '{{ url("resort/performance/pip") }}/' + id,
                type: 'DELETE',
