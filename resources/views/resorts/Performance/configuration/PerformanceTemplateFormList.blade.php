        ...
        <div class="modal-content">
            <form id="jobDesEditForm" data-parsley-validate>
                ...
                <div class="modal-header">
                    <h5 class="modal-title" id="staticBackdropLabel">Edit Form Template</h5>
      ...
      $(document).ready(function ()
{
    ...
    // Per-field "Responder Roles" — fan out to every field type so HR can
    // pick which role(s) are allowed to fill each field.
    ...
    $(document).on("click", ".edit-row-btn", function(){
        ...
        $.ajax({
            url: "{{ route('Performance.config.GetPerformanceTemplateForm', ['id' => '__ID__']) }}".replace('__ID__', id),
            type: "get",
            data: {
                id: id,
            },
    ...
    $('#save-form').click(function (e)
    {
        ...
        e.preventDefault();
        $.ajax({
            url: "{{ route('Performance.config.PerformanceTemplateFormUpdate') }}",
            type: "POST",
            data: {
                FormName: formname,
        ...
        function ReviewType()
        {
            ...
            var TableAccomMainten = $('#table-PerformanceTemplateFormList').DataTable({
                ...
                order:[[5,'desc']],
                ajax: {
                    url: '{{ route("Performance.config.PerformanceTemplateFormList") }}',
                    type: 'GET',
                    data: function (d) {
