    ...
    <div class="container-fluid">
        ...
        <div class="row g-3 g-xxl-4 card-heigth">
            ...
            <div class="col-xl-6 @if(App\Helpers\Common::checkRouteWisePermission('leave.request',config('settings.resort_permissions.view')) == false) d-none @endif">
                <div class="card h-auto" id="card-chartLeave">
                    <div class="card-title">
                        <div class="row justify-content-between align-items-center g-1">
                            <div class="col">
                                <h3 class="text-nowrap">Leave History</h3>
                            </div>
                            <div class="col-auto">
                                <div class="form-group">
            ...
            <div class="col-lg-4 col-md-6">
                        <div class="card card-wiINsight wai-narrative">
                            ...
                            <div class="wai-head">
                                ...
                                @if($meta)
                                    <div class="wai-head-meta">
                                        ...
                                        @else
                                            <span title="{{ $meta['next_available']->format('d M Y, H:i') }}">&middot; Regenerate available {{ $meta['next_available']->diffForHumans() }}</span>
                                        @endif
                                    </div>
                                @endif
                            </div>
                            <div class="leaveUser-main wai-narrative-body">
                                @php
                                    $leaveInsightRows = [
                                        ['key' => 'occupancy', 'default' => 'Occupancy & Leave Window',       'modal' => 'leaveInsightOccupancyModal'],
                                        ['key' => 'peak',      'default' => 'AI Forecasted Peak Leave Periods', 'modal' => 'leaveInsightPeakModal'],
                                        ['key' => 'behavior',  'default' => 'Employee Leave Behavior Analysis', 'modal' => 'leaveInsightBehaviorModal'],
                                    ];
                                @endphp
                                @foreach ($leaveInsightRows as $lc)
                                    @php $hasRecommendation = !empty($leaveInsights[$lc['key']]['recommendation']); @endphp
                                    <div class="wai-row">
...
<style>
    /* WAI Insights — same gradient header as the other WAI Insights cards
       (Time and Attendance, Payroll, Talent Acquisition). This card's 3
       checks (occupancy, peak, behavior) are narrative AI insights — title
       + descriptive body + optional recommendation — not pass/fail
       compliance counts, so no hero/count here either, same reasoning as
       the other narrative-style cards: icon is amber when there's a
       recommendation worth acting on, teal tick when the insight is
       purely informational. No fixed card height here — unlike Payroll/TA,
       this card never had one to preserve. */
    .card-wiINsight.wai-narrative { padding: 0; overflow: hidden; border-radius: 16px; }
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    callback({
                        ...
                        permission: response.permission // Pass permissions to the DataTable
                    });
                }
            });
        },
...
<script type="module">
    document.addEventListener('DOMContentLoaded', function () {
        document.body.addEventListener('click', function (e) {
            if (e.target && e.target.classList.contains('notify-birthday')) {
                const employeeId = e.target.getAttribute('data-employee-id');
                console.log('Notify button clicked for employee ID:', employeeId);

                // Call the function to send notifications
                sendBirthdayNotification(employeeId);
            }
        });
    });

    const routes = {
        sendBirthdayNotification: "{{ route('birthday.notification.send') }}"
    };
    function sendBirthdayNotification(employeeId) {
        fetch(routes.sendBirthdayNotification, {
            method: 'POST',
            headers: {
            ...
            .then(data => {
                if (data.status === 'success') {
                    alert('Birthday notifications sent successfully!');
                } else {
