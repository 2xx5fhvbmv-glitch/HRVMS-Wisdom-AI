                ...
                <!-- <div class="col-xxl-6 col-xl-6 col-lg-6"> -->
                    <div class="card">
                        ...
                        <table id="DisciplineryCategory" class="table  w-100">
                            <thead>
                                <tr>
                                    <th class="text-nowrap">Disciplinary Name</th>
                                    <th>Description</th>
...
<script>
        var DisciplineryCategory = $('#DisciplineryCategory').DataTable({
            ...
            serverSide: true,
            order:[[3, 'desc']],
            ajax: '{{ route("GrievanceAndDisciplinery.config.Disciplineryindex") }}',
            columns: [
                { data: 'DisciplinaryCategoryName', name: 'DisciplinaryName', className: 'text-nowrap' },
        ...
        $(document).on('click', '.delete-row-btn', function (e) {
            ...
            wisdomConfirm({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: "DELETE",
                        url: "{{ route('GrievanceAndDisciplinery.config.DisciplineryCategoryDestory', ':id') }}".replace(':id', main_id),
                        dataType: "json",
        ...
        $(document).on("click", "#DisciplineryCategory .update-row-btn_cat", function (event) {
            ...
            $.ajax({
                url: "{{ route('GrievanceAndDisciplinery.config.DisciplineryCategoryinlineUpdate', '__id__') }}".replace('__id__', Main_id),
                type: "PUT",
                data: {
                    Main_id:Main_id,
                    DisciplinaryCategoryName:DiscriplineryName,
