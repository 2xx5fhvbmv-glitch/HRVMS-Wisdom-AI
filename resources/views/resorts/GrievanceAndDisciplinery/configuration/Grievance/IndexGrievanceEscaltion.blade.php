...
<script>
        var DisciplineryCategory = $('#IndexGrievanceDelegationRule').DataTable({
            ...
            serverSide: true,
            order:[[3, 'desc']],
            ajax: '{{ route("GrievanceAndDisciplinery.config.IndexGrievanceEscaltion") }}',
            columns: [
                { data: 'Grievance_Cat_id', name: 'Grievance_Cat_id', className: 'text-nowrap' },
                { data: 'resolved_duration', name: 'resolved_duration', className: 'text-nowrap' },
        ...
        $(document).on('click', '.delete-row-btn', function (e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: "DELETE",
                        url: "{{ route('GrievanceAndDisciplinery.config.GrievanceEscalationDestory', ':id') }}".replace(':id', main_id),
                        dataType: "json",
