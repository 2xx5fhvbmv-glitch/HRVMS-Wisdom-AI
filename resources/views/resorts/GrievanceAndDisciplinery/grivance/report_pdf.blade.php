<!DOCTYPE html>
{{--
    Grievance Case Report — §6.14 of the e-signature spec.
    Frozen signature of whoever marked the case resolved (see
    GrivanceController::InvestigationReportStore() /
    downloadGrievanceReportPdf()), never re-derived from a live
    signature_img. Gated on GrivanceSubmissionModel.status === 'resolved'.
--}}
@php
...
<head>
    <meta charset="utf-8">
    <title>Grievance Case Report</title>
    <style>
...
<body>
    ...
    <div class="meta">
        <strong>Ref:</strong> GRV/{{ $grievance->id }}/{{ \Carbon\Carbon::now()->format('Y') }}<br>
        <strong>Date:</strong> {{ \Carbon\Carbon::now()->format('d M Y') }}
    </div>

    <div class="title">GRIEVANCE CASE REPORT</div>

    <table class="details">
        <tr><td class="label">Employee</td><td>{{ optional(optional($grievance->GetEmployee)->resortAdmin)->full_name ?? '—' }}</td></tr>
        <tr><td class="label">Category</td><td>{{ optional($grievance->category)->Category_Name ?? '—' }}</td></tr>
        <tr><td class="label">Description</td><td>{{ $grievance->Grivance_description ?? '—' }}</td></tr>
        <tr><td class="label">Location</td><td>{{ $grievance->location ?? '—' }}</td></tr>
        <tr><td class="label">Outcome</td><td>{{ $grievance->outcome_type ?? '—' }}</td></tr>
        <tr><td class="label">Action Taken</td><td>{{ $grievance->action_taken ?? '—' }}</td></tr>
        <tr><td class="label">Status</td><td>{{ ucfirst($grievance->status ?? '—') }}</td></tr>
    </table>
    ...
    <p class="footer-note">
        This is a system-generated grievance case report. For queries, please contact the HR Department.
    </p>
