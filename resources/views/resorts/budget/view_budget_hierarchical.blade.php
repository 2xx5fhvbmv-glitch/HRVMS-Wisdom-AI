        ...
        <div class="card">
            <div class="card-header">
                <div class="row g-md-3 g-2 align-items-center">
                    ...
                    {{-- Copy budget from another year. Opens a small inline
                         form to pick a SOURCE year, then POSTs to the
                         clone endpoint. The endpoint is idempotent so HR
                         can safely re-run if they made manual edits
                         after the first clone. --}}
                    <div class="col-xl-auto col-md-auto col-sm-auto col-12 ms-auto">
                        <button type="button" class="btn btn-outline-primary btn-sm" data-bs-toggle="modal" data-bs-target="#copyBudgetModal">
                            <i class="fa-solid fa-copy me-1"></i> Copy from previous year
                        </button>
                    </div>
                </div>
            </div>

            {{-- Copy-budget modal: pick source year, confirms before
                 firing the POST. Dest year is the currently-selected
                 year filter so HR doesn't accidentally overwrite the
                 wrong target. --}}
            <div class="modal fade" id="copyBudgetModal" tabindex="-1" aria-labelledby="copyBudgetLabel" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="copyBudgetLabel">Copy budget into {{ $selectedYear }}</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <p class="mb-3">
                                Copies per-employee monthly salary overrides, per-employee cost overrides, vacant slots and their cost overrides from the source year into <strong>{{ $selectedYear }}</strong>. Cost templates, employee allowances and the Basic Salary on each employee are not year-keyed so they already apply automatically.
                            </p>
                            <div class="alert alert-info py-2 mb-3" style="font-size: 13px;">
                                <i class="fa-solid fa-info-circle me-1"></i>
                                Safe to re-run. Rows you've already edited in {{ $selectedYear }} won't be overwritten — only missing months / costs get copied.
                            </div>
                            <label for="copyBudgetFromYear" class="form-label">Source year</label>
                            <select id="copyBudgetFromYear" class="form-select">
                                @for ($cy = $endYear; $cy >= $startYear; $cy--)
                                    @if ($cy != $selectedYear)
                                        <option value="{{ $cy }}" @if($cy == $selectedYear - 1) selected @endif>{{ $cy }}</option>
                                    @endif
                                @endfor
                            </select>
                        </div>
                        <div class="modal-footer">
                            ...
                            <button type="button" class="btn btn-primary btn-sm" id="copyBudgetConfirmBtn" data-target-year="{{ $selectedYear }}">
                                Copy
                            </button>
                        </div>
                    </div>
            ...
            <script>
                // Copy-budget submit handler. Disables the button while the
                // POST is in flight, surfaces server messages via toastr,
                // and reloads the page so the freshly-copied rows appear in
                // the budget tree without HR having to refresh manually.
                document.addEventListener('DOMContentLoaded', function () {
                    var btn = document.getElementById('copyBudgetConfirmBtn');
                    if (!btn) return;
                    btn.addEventListener('click', function () {
                        var fromYear   = document.getElementById('copyBudgetFromYear').value;
                        var targetYear = btn.getAttribute('data-target-year');
                        if (!fromYear || !targetYear) return;
                        btn.disabled = true;
                        btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin me-1"></i> Copying…';
                        fetch('{{ route("resort.budget.clone-from-year") }}', {
                            method: 'POST',
                            headers: {
                            ...
                            body: JSON.stringify({ from_year: fromYear, to_year: targetYear })
                        }).then(function (r) {
                            ...
                            if (res.ok && res.body.success) {
                                if (typeof toastr !== 'undefined') {
                                    toastr.success(res.body.message, 'Budget copied', { positionClass: 'toast-bottom-right', timeOut: 5000 });
                                }
                                // Hide modal then reload so the new
                                // rows show up in the tree.
                                var modalEl = document.getElementById('copyBudgetModal');
                                var modal = bootstrap.Modal.getInstance(modalEl);
                                if (modal) modal.hide();
                                setTimeout(function () { window.location.reload(); }, 900);
                            } else {
                                if (typeof toastr !== 'undefined') {
                                    toastr.error(res.body.message || 'Copy failed.', 'Error', { positionClass: 'toast-bottom-right' });
                                }
                            }
                        }).catch(function () {
                            btn.disabled = false;
                            btn.innerHTML = 'Copy';
                            if (typeof toastr !== 'undefined') {
                                toastr.error('Network error — copy not completed.', 'Error', { positionClass: 'toast-bottom-right' });
                            }
                        });
                    });
                });
