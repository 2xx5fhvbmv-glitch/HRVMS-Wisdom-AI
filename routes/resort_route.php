...
Route::prefix('resort')->middleware(['auth:resort-admin','revalidate','checkResortPermission'])->namespace('Resorts')->group(function () {
    ...
    Route::get( '/talent-acquisition/view-questions/{id}', ['App\Http\Controllers\Resorts\TalentAcquisition\QuestionnaireController','show'])->name('resort.ta.Questions.show');
   ...
   //Leave Module Start
    ...
    Route::get('accommodation/get-employee-accommodation', 'Accommodation\MaintananceContorller@getEmployeeAccommodation')->name('resort.accommodation.getEmployeeAccommodation');
   ...
   Route::post('/incidents/approve-or-reject', 'Incident\IncidentController@approveOrReject')->name('incident.investigation.approvedorreject');
    ...
    ->name('people.advance-salary-repayment-tracker.downloadPdf');
      ...
      Route::get('/people/employees/grid', 'People\Employee\EmployeeController@fetchEmployeesGrid')->name('people.employees.grid');
       ...
       // Update profile / full-length photo on an existing employee. Plugs
       // the gap that forced HR to re-create an employee just to replace a
       // wrongly-uploaded picture (DR-485 on live, PDF saved as photo).
       Route::post('/people/employees/update-profile-picture', 'People\Employee\EmployeeController@updateProfilePicture')->name('people.employees.updateProfilePicture');
    ...
    Route::post('report/workforce-planning/export', 'WorkforcePlanningReportController@export')->name('resort.report.wfp.export');
    Route::post('report/workforce-planning/insights', 'WorkforcePlanningReportController@insights')->name('resort.report.wfp.insights');
    ...
    // Predefined Grievance & Disciplinary reports (Option B).
    Route::get('report/grievance', 'GrievanceReportController@index')->name('resort.report.grievance.index');
    Route::post('report/grievance/run', 'GrievanceReportController@run')->name('resort.report.grievance.run');
    ...
    // Predefined People Management – Onboarding reports (Option B).
    Route::get('report/onboarding', 'OnboardingReportController@index')->name('resort.report.onboarding.index');
    Route::post('report/onboarding/run', 'OnboardingReportController@run')->name('resort.report.onboarding.run');
    Route::post('report/onboarding/export', 'OnboardingReportController@export')->name('resort.report.onboarding.export');
    Route::post('report/onboarding/insights', 'OnboardingReportController@insights')->name('resort.report.onboarding.insights');
